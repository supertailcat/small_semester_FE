    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{
            scope.row.eventDate | datefmt("YYYY-MM-DD hh:mm:ss")
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
...
<script>
import AddOrUpdate from "./eventinfo-add-or-update";
export default {
  data() {
    return {
      eventroom: new Map(),
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl("/event/eventinfo/list"),
        method: "get",
        params: this.$http.adornParams({
          page: this.pageIndex,
          limit: this.pageSize,
    ...
    deleteHandle(id) {
      ...
      this.$confirm(
        ...
        this.$http({
          url: this.$http.adornUrl("/event/eventinfo/delete"),
          method: "post",
